...
import { evidenceLabels, lessons, quiz, ui, weeks, type Locale, type Surface } from "./atlas-data";
...
function Lesson({ locale, slug, progress, toggle }: { locale: Locale; slug: string; progress: ProgressState; toggle: (id: string) => void }) {
  ...
  return <article className="lesson-page"><div className="lesson-hero"><p className="eyebrow">{lesson.eyebrow}</p><EvidencePill locale={locale} level={lesson.evidence} /><h1>{lesson.title}</h1><p>{lesson.summary}</p></div><div className="lesson-body"><main>{lesson.body.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<section className="learning-transform"><div><span>01</span><p className="kicker">{tr ? "İlk düşünce" : "First thought"}</p><p>{lesson.misconception}</p></div><div><span>02</span><p className="kicker">{tr ? "Düzeltme" : "Correction"}</p><p>{lesson.correction}</p></div><div><span>03</span><p className="kicker">{tr ? "Karar kuralı" : "Decision rule"}</p><p>{lesson.decision}</p></div></section><button className={done ? "completed-button" : ""} onClick={() => toggle(lesson.id)}>{done ? (tr ? "Tamamlandı olarak işaretlendi" : "Marked complete") : (tr ? "Bu dersi tamamla" : "Complete this lesson")}</button></main><aside><h2>{tr ? "Kendini kontrol et" : "Check yourself"}</h2><ul>{lesson.checks.map((check) => <li key={check}>{check}</li>)}</ul><p className="verified-at">{tr ? "Doğrulama tarihi" : "Verified"}: {lesson.verifiedAt}</p></aside></div><nav className="lesson-nav">{lessons[locale].filter((item) => item.id !== lesson.id).slice(0, 3).map((item) => <Link key={item.id} href={`/${locale}/learn/${item.slug}/`}>{item.title} →</Link>)}</nav></article>;
}
