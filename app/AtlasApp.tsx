  ...
  return (
    <aside className="analogy-card" aria-label={tr ? "Günlük hayat analojisi" : "Everyday analogy"}>
      <div className="analogy-head">
        <span className="analogy-emoji" aria-hidden>💡</span>
  ...
  return (
    <aside className="why-matters" aria-label={tr ? "Neden önemli" : "Why it matters"}>
      <div className="why-head">
        <span className="why-emoji" aria-hidden>🎯</span>
...
function ConceptDiagram({ kind }: { kind: DiagramKey }) {
  ...
  if (kind === "models") {
    return (
      <svg viewBox="0 0 320 200" role="img" aria-label="Model türleri şeması" className="concept-svg">
        ...
        <g fontFamily="monospace" fontSize="11" fill={ink}>
          ...
          <text x="80" y="70" textAnchor="middle" fill={muted}>ham ön-eğitim</text>
          ...
          <text x="235" y="70" textAnchor="middle" fill={muted}>talimatlara hizalı</text>
          ...
          <text x="160" y="160" textAnchor="middle" fill={muted}>adım adım düşünür</text>
  ...
  if (kind === "tokens") {
    return (
      <svg viewBox="0 0 320 200" role="img" aria-label="Token ve attention şeması" className="concept-svg">
        <rect x="0" y="0" width="320" height="200" fill="var(--surface)" stroke={line} rx="10" />
        <g fontFamily="monospace" fontSize="11">
          ...
          <text x="20" y="175" fill={ink} fontSize="11">KV cache → önceki K/V belleği</text>
        </g>
      </svg>
    );
  }
  ...
  if (kind === "lora") {
    return (
      <svg viewBox="0 0 320 200" role="img" aria-label="LoRA ve QLoRA şeması" className="concept-svg">
        <rect x="0" y="0" width="320" height="200" fill="var(--surface)" stroke={line} rx="10" />
        <g fontFamily="monospace" fontSize="11">
          ...
          <text x="55" y="105" textAnchor="middle" fill={muted}>donuk</text>
          ...
          <text x="100" y="155" textAnchor="middle" fill={muted} fontSize="10">LoRA: yalnız A ve B eğitilir</text>
  ...
  if (kind === "rank") {
    return (
      <svg viewBox="0 0 320 200" role="img" aria-label="Rank ve alpha şeması" className="concept-svg">
        <rect x="0" y="0" width="320" height="200" fill="var(--surface)" stroke={line} rx="10" />
        <g fontFamily="monospace" fontSize="11">
          ...
          <text x="20" y="170" fill={ink} fontSize="10">r = rank (küçük tut) | α = ölçek (α/r LoRA, α/√r rsLoRA)</text>
        </g>
      </svg>
    );
  }
  ...
  if (kind === "steps") {
    return (
      <svg viewBox="0 0 320 200" role="img" aria-label="Eğitim döngüsü" className="concept-svg">
        <rect x="0" y="0" width="320" height="200" fill="var(--surface)" stroke={line} rx="10" />
        <g fontFamily="monospace" fontSize="10">
          ...
          <text x="265" y="103" fill={muted}>tekrar</text>
          ...
          <text x="160" y="180" textAnchor="middle" fill={muted}>ağırlıkları 1 kez günceller</text>
        </g>
      </svg>
    );
  }
  ...
  if (kind === "loss") {
    ...
    return (
      <svg viewBox="0 0 320 200" role="img" aria-label="Loss eğrileri" className="concept-svg">
        <rect x="0" y="0" width="320" height="200" fill="var(--surface)" stroke={line} rx="10" />
        <g fontFamily="monospace" fontSize="10">
          ...
          <text x="240" y="85" fill={red} fontSize="10">val</text>

          <circle cx="220" cy="135" r="4" fill={yellow} />
          <text x="120" y="195" textAnchor="middle" fill={yellow} fontSize="10">↑ overfitting başlangıcı</text>
        </g>
      </svg>
    );
  }
  ...
  if (kind === "templates") {
    ...
    return (
      <svg viewBox="0 0 320 200" role="img" aria-label="Chat template pipeline" className="concept-svg">
        <rect x="0" y="0" width="320" height="200" fill="var(--surface)" stroke={line} rx="10" />
        <g fontFamily="monospace" fontSize="10">
          {[
          ...
          <text x="180" y="55" textAnchor="middle" fill={ink} fontSize="9">template</text>
          <text x="180" y="70" textAnchor="middle" fill={muted} fontSize="9">render</text>
          ...
          <text x="20" y="120" fill={ink} fontSize="10">Mask: -100 (prompt) | gerçek id (assistant)</text>
          <text x="20" y="140" fill={green} fontSize="10">Doğru mask → yalnız cevap loss&apos;a katkı sağlar</text>
          <text x="20" y="158" fill={red} fontSize="10">Yanlış mask → cevap loss dışında kalabilir</text>
        </g>
      </svg>
    );
  }
  ...
  return (
    <svg viewBox="0 0 320 200" role="img" aria-label="Değerlendirme metrikleri" className="concept-svg">
      <rect x="0" y="0" width="320" height="200" fill="var(--surface)" stroke={line} rx="10" />
      <g fontFamily="monospace" fontSize="10">
        ...
        {/* Etiketler */}
        <text x="160" y="32" textAnchor="middle" fill={ink}>Domain</text>
        <text x="240" y="84" textAnchor="middle" fill={ink}>Format</text>
        <text x="215" y="175" textAnchor="middle" fill={ink}>Safety</text>
        <text x="105" y="175" textAnchor="middle" fill={ink}>Retention</text>
        <text x="80" y="84" textAnchor="middle" fill={ink}>Uncertainty</text>

        <text x="20" y="195" fill={muted}>Ağırlıklı ortalama = (D·.35) + (F·.15) + (S·.20) + (U·.15) + (R·.15)</text>
      </g>
    </svg>
  );
}
  ...
  return (
    <section className="surface-stack">
      ...
      <div className="lab-grid">
        <article className="lab-card wide"><div><span className="lab-index">01</span><h2>{tr ? "Context bütçesi" : "Context budget"}</h2></div><div className="input-grid"><LabInput label="Maximum" value={get("contextMax", 4096)} onChange={(v) => setLab("contextMax", v)} /><LabInput label="System" value={get("systemTokens", 200)} onChange={(v) => setLab("systemTokens", v)} /><LabInput label="Template" value={get("templateTokens", 80)} onChange={(v) => setLab("templateTokens", v)} /><LabInput label="Input" value={get("inputTokens", 900)} onChange={(v) => setLab("inputTokens", v)} /><LabInput label="RAG" value={get("ragTokens", 1000)} onChange={(v) => setLab("ragTokens", v)} /><LabInput label="Response" value={get("responseTokens", 600)} onChange={(v) => setLab("responseTokens", v)} /></div><div className={`result ${context.fits ? "good" : "bad"}`}><strong>{context.used.toLocaleString(locale)} tokens</strong><span>{context.fits ? `${context.remaining.toLocaleString(locale)} ${tr ? "token boş" : "tokens free"}` : `${Math.abs(context.remaining).toLocaleString(locale)} ${tr ? "token taşma" : "tokens over"}`}</span></div></article>

        <article className="lab-card"><div><span className="lab-index">02</span><h2>{tr ? "Batch ve step" : "Batch and steps"}</h2></div><div className="input-grid"><LabInput label={tr ? "Örnek" : "Examples"} value={get("dataset", 2000)} onChange={(v) => setLab("dataset", v)} /><LabInput label="Micro batch" value={get("micro", 1)} onChange={(v) => setLab("micro", v)} min={1} /><LabInput label="Accumulation" value={get("accum", 16)} onChange={(v) => setLab("accum", v)} min={1} /><LabInput label="GPU" value={get("gpus", 1)} onChange={(v) => setLab("gpus", v)} min={1} /><LabInput label="Epoch" value={get("epochs", 3)} onChange={(v) => setLab("epochs", v)} min={1} /></div><dl className="metrics"><div><dt>Effective batch</dt><dd>{batch.effectiveBatch}</dd></div><div><dt>{tr ? "Toplam optimizer step" : "Total optimizer steps"}</dt><dd>{batch.totalOptimizerSteps}</dd></div></dl></article>

        <article className="lab-card"><div><span className="lab-index">03</span><h2>LoRA scale</h2></div><div className="input-grid"><LabInput label="Rank" value={rank} onChange={(v) => setLab("rank", v)} min={1} /><LabInput label="Alpha" value={alpha} onChange={(v) => setLab("alpha", v)} /><LabInput label={tr ? "Matris sayısı" : "Matrices"} value={matrices} onChange={(v) => setLab("matrices", v)} min={1} /><LabInput label={tr ? "Boyut" : "Dimension"} value={dimension} onChange={(v) => setLab("dimension", v)} min={1} /></div><dl className="metrics"><div><dt>LoRA α/r</dt><dd>{loraScale(rank, alpha).toFixed(3)}</dd></div><div><dt>rsLoRA α/√r</dt><dd>{loraScale(rank, alpha, true).toFixed(3)}</dd></div><div><dt>{tr ? "Adapter parametresi" : "Adapter parameters"}</dt><dd>{loraParameterCount(rank, dimension, dimension, matrices).toLocaleString(locale)}</dd></div></dl></article>

        <article className="lab-card"><div><span className="lab-index">04</span><h2>{tr ? "Türkçe token ölçümü" : "Turkish token measurement"}</h2></div><p>{tr ? "Sonuç uydurulmaz. Aynı metni iki gerçek tokenizer ile ölçüp değerleri gir." : "No result is invented. Measure the same text with two real tokenizers and enter the values."}</p><div className="input-grid"><LabInput label={tr ? "Base tokenizer" : "Base tokenizer"} value={baseTokens} onChange={(v) => setLab("baseTokens", v)} /><LabInput label={tr ? "Aday tokenizer" : "Candidate tokenizer"} value={candidateTokens} onChange={(v) => setLab("candidateTokens", v)} /></div><div className="result"><strong>{baseTokens > 0 && candidateTokens > 0 ? `${((candidateTokens / baseTokens - 1) * 100).toFixed(1)}%` : "—"}</strong><span>{tr ? "Adayın base'e göre token farkı" : "Candidate token delta vs base"}</span></div></article>

        <article className="lab-card wide"><div><span className="lab-index">05</span><h2>{tr ? "Dataset karışımı" : "Dataset mix"}</h2></div><div className="input-grid"><LabInput label={tr ? "Toplam örnek" : "Total examples"} value={get("mixTotal", 2000)} onChange={(v) => setLab("mixTotal", v)} /><LabInput label={tr ? "Standart %" : "Standard %"} value={mixPercentages.standard} onChange={(v) => setLab("mixStandard", v)} /><LabInput label={tr ? "Parafraz %" : "Paraphrase %"} value={mixPercentages.paraphrase} onChange={(v) => setLab("mixParaphrase", v)} /><LabInput label={tr ? "Eksik bilgi %" : "Missing info %"} value={mixPercentages.missing} onChange={(v) => setLab("mixMissing", v)} /><LabInput label={tr ? "Negatif %" : "Negative %"} value={mixPercentages.negative} onChange={(v) => setLab("mixNegative", v)} /><LabInput label={tr ? "Eskalasyon %" : "Escalation %"} value={mixPercentages.escalation} onChange={(v) => setLab("mixEscalation", v)} /></div><div className={`result ${mix.valid ? "good" : "bad"}`}><strong>{mix.sum}%</strong><span>{Object.entries(mix.counts).map(([key, value]) => `${key} ${value}`).join(" · ")}</span></div></article>

        <article className="lab-card wide"><div><span className="lab-index">06</span><h2>{tr ? "Ağırlıklı benchmark" : "Weighted benchmark"}</h2></div><div className="input-grid"><LabInput label="Domain 35%" value={get("scoreDomain", 70)} onChange={(v) => setLab("scoreDomain", v)} max={100} /><LabInput label="Format 15%" value={get("scoreFormat", 80)} onChange={(v) => setLab("scoreFormat", v)} max={100} /><LabInput label="Safety 20%" value={get("scoreSafety", 90)} onChange={(v) => setLab("scoreSafety", v)} max={100} /><LabInput label="Uncertainty 15%" value={get("scoreUncertainty", 70)} onChange={(v) => setLab("scoreUncertainty", v)} max={100} /><LabInput label="Retention 15%" value={get("scoreRetention", 85)} onChange={(v) => setLab("scoreRetention", v)} max={100} /></div><div className="score-orb"><strong>{benchmark.toFixed(1)}</strong><span>/ 100</span></div></article>

        <article className="lab-card wide"><div><span className="lab-index">07</span><h2>{tr ? "Template ve masking kontrolü" : "Template and masking check"}</h2></div><ol className="checklist"><li>{tr ? "Tek bir örneği gerçek tokenizer ile render et." : "Render one example with the real tokenizer."}</li><li>{tr ? "BOS, role sınırları, assistant başlangıcı ve EOS'u görünür kıl." : "Expose BOS, role boundaries, assistant start, and EOS."}</li><li>{tr ? "Labels içinde prompt tokenlarının -100, cevap tokenlarının aktif olduğunu doğrula." : "Verify prompt labels are -100 while response labels remain active."}</li><li>{tr ? "Inference'ta aynı template ve stop davranışını kullan." : "Use the same template and stop behavior at inference."}</li></ol></article>

        <article className="lab-card wide"><div><span className="lab-index">08</span><h2>{tr ? "4070 Ti Super planlama rehberi" : "4070 Ti Super planning guide"}</h2></div><div className="warning"><EvidencePill locale={locale} level="simulation" /><p>{tr ? "16 GB için güvenli başlangıç: 4-bit base, micro batch 1, context 1024–2048, gradient checkpointing. OOM sırası: batch → context → daha küçük model → offload. Bu kart gerçek bir koşunun kanıtı değildir." : "Safe 16 GB starting point: 4-bit base, micro batch 1, context 1024–2048, gradient checkpointing. OOM order: batch → context → smaller model → offload. This card is not evidence of a real run."}</p></div></article>
      </div>
    </section>
  );
}
...
function Evidence({ locale }: { locale: Locale }) {
  ...
  return <section className="surface-stack"><div className="page-intro"><p className="eyebrow">QWEN3 4B · FINETOME</p><h1>{tr ? "Kanıt kaydı" : "Evidence record"}</h1><p>{tr ? "Başarılı pipeline ile başarılı model kalitesini ayıran deney günlüğü." : "An experiment record that separates a working pipeline from improved model quality."}</p></div><div className="evidence-summary"><article><EvidencePill locale={locale} level="verified" /><b>30 / 30</b><span>{tr ? "eğitim adımı tamamlandı" : "training steps completed"}</span></article><article><EvidencePill locale={locale} level="verified" /><b>0.8245</b><span>{tr ? "son training loss" : "final training loss"}</span></article><article><EvidencePill locale={locale} level="verified" /><b>✓</b><span>{tr ? "adapter kaydetme ve reload" : "adapter save and reload"}</span></article><article><EvidencePill locale={locale} level="observed" /><b>9.62 / 15.99 GiB</b><span>{tr ? "gözlenen anlık kullanım; peak değil" : "observed snapshot; not peak"}</span></article></div><article className="verdict"><div><p className="eyebrow">{tr ? "KARAR" : "VERDICT"}</p><h2>{tr ? "Pipeline geçti. Kalite kazanımı kanıtlanmadı." : "Pipeline passed. Quality gain was not proven."}</h2></div><p>{tr ? "Evaluation split yoktu ve üç sabit kalite karşılaştırmasının tamamında base model daha güçlüydü. Bu koşu bir smoke test kanıtıdır; model iyileşmesi kanıtı değildir." : "There was no evaluation split, and the base model was stronger in all three fixed quality comparisons. This run proves a smoke-test pipeline, not a model improvement."}</p></article><div className="comparison-table" role="table" aria-label={tr ? "Base ve fine-tuned karşılaştırması" : "Base and fine-tuned comparison"}>{comparisons.map((row) => <div role="row" key={row[0]}><b role="cell">{row[0]}</b><span role="cell">{row[1]}</span><span role="cell">{row[2]}</span></div>)}</div><div className="unknowns"><h2>{tr ? "Bilinmeyenler" : "Unknowns"}</h2><ul><li>{tr ? "Gerçek peak VRAM ölçülmedi." : "Real peak VRAM was not measured."}</li><li>{tr ? "Bağımsız validation/test split yoktu." : "There was no independent validation/test split."}</li><li>{tr ? "Sabit seed ve tekrar koşusu yoktu." : "There was no fixed seed or repeated run."}</li><li>{tr ? "Kalite farkının istatistiksel güveni bilinmiyor." : "Statistical confidence of the quality delta is unknown."}</li></ul></div></section>;
}
...
function DataEvaluation({ locale, progress, onFinish }: { locale: Locale; progress: ProgressState; onFinish: (score: number, gaps: string[]) => void }) {
  const tr = locale === "tr";
  return <section className="surface-stack"><div className="page-intro"><p className="eyebrow">{tr ? "VERİ → BENCHMARK → KARAR" : "DATA → BENCHMARK → DECISION"}</p><h1>{tr ? "Veri ve değerlendirme" : "Data and evaluation"}</h1><p>{tr ? "Leakage, biçim ve güvenlik kontrolleri loss'tan önce tasarlanır." : "Leakage, formatting, and safety checks are designed before looking at loss."}</p></div><div className="recipe-grid">{(tr ? [["55%", "Standart", "Temel görev dağılımı"], ["10%", "Parafraz", "Yüzey çeşitliliği"], ["15%", "Eksik bilgi", "Belirsizlik davranışı"], ["10%", "Negatif", "Yanlış varsayıma direnç"], ["10%", "Eskalasyon", "Güvenli yönlendirme"]] : [["55%", "Standard", "Core task distribution"], ["10%", "Paraphrase", "Surface variation"], ["15%", "Missing info", "Uncertainty behavior"], ["10%", "Negative", "Resistance to false premises"], ["10%", "Escalation", "Safe redirection"]]).map(([amount, title, description]) => <article key={title}><b>{amount}</b><h3>{title}</h3><p>{description}</p></article>)}</div><div className="gate-grid"><article><h2>{tr ? "Dataset kapısı" : "Dataset gate"}</h2><ul className="checklist"><li>{tr ? "Train/validation/test kaynakları ayrık" : "Train/validation/test sources are disjoint"}</li><li>{tr ? "Duplicate ve near-duplicate taraması" : "Duplicate and near-duplicate scan"}</li><li>{tr ? "PII ve özel operasyon ayrıntıları temiz" : "PII and private operational details removed"}</li><li>{tr ? "Chat template gerçek tokenizer ile inspect edildi" : "Chat template inspected with the real tokenizer"}</li></ul></article><article><h2>{tr ? "Model kabul kapısı" : "Model acceptance gate"}</h2><ul className="checklist"><li>Domain ≥ baseline</li><li>Format ≥ 95</li><li>Safety ≥ baseline</li><li>Retention drop ≤ 3</li><li>{tr ? "Aynı seed ve generation ayarları" : "Same seed and generation settings"}</li></ul></article></div><MixedQuiz locale={locale} progress={progress} onFinish={onFinish} /></section>;
}
