...
test("renders bilingual dashboards with source and visitor progress separated", async () => {
  ...
  assert.match(tr, /<title>Panel · Unsloth Studio Learning Atlas<\/title>/);
  assert.match(en, /<title>Dashboard · Unsloth Studio Learning Atlas<\/title>/);
...
test("renders labs, evidence, lessons, and localized metadata", async () => {
  ...
  assert.match(labsHtml, /Etkileşimli Laboratuvarlar · Unsloth Studio Learning Atlas/);
